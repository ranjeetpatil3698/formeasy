...
function NumberClient({id,label,important,solution,done}) {
    ...
    return (
        ...
        <label className="font-meduim text-3xl w-full outline-none h-12" >{label}</label>
        <input className="border-2 border-gray-400 rounded-sm max-w-xs  focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-transparent" type="number" value={solution} onChange={handleAnswerChange} />
